...
```bash
curl -s --max-time 30 ${JINA_API_KEY:+-H} ${JINA_API_KEY:+"Authorization: Bearer $JINA_API_KEY"} "https://r.jina.ai/TARGET_URL"
```

`JINA_API_KEY` is exported from `~/.zshrc`. When set, the auth header lifts the rate limit from the anonymous tier to the keyed tier; when unset, both `${VAR:+...}` expansions vanish (split into two so zsh and bash both produce separate `-H` / header args) and the request runs anonymously.
...
for attempt in 1 2 3; do
  result=$(curl -s --max-time 30 ${JINA_API_KEY:+-H} ${JINA_API_KEY:+"Authorization: Bearer $JINA_API_KEY"} "https://r.jina.ai/TARGET_URL" 2>&1)
